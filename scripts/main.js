...
function connect(){
    ...
    websocket.onclose = function (evt) {
        ...
        //add the realTime Button control
        $("#realTime").get(0).disabled = true;
        //add the runTicker Button control
        $("#runTicker").get(0).disabled = true;
    };
    //websocket.onerror()
...
function logon(){
    ...
    logon.client_id = "WebApiTest";// required, "WebApiTest" is the only id for individual user
    ...
    // $("#logon").get(0).disabled = true;
    // $("#logoff").get(0).disabled = false;
    //add below code to enable the Metadata Request Button
...
function processMessage(msg) {
    ...
    if (sMsg.logon_result)
        processLogonResult(sMsg.logon_result);//see function in line 164
    //add processInformationReport function to separate Metadata message
    ...
    //add processRealTimeMarketData function to separate real_time_market_data message
    for (var i = 0; i < sMsg.real_time_market_data.length; ++i)
        //this function is in metareal.js and line xxxx
